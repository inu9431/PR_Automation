...
from app.services.reviewer import build_discord_message, calculate_cost
...
async def test_review_pr_claude_fail(db):
    with (
        patch("app.services.reviewer.get_pr_diff_text", return_value="diff text"),
        patch(
            "app.services.reviewer.client.messages.create", side_effect=Exception("API key expired")
        ),
    ):
        from app.services.reviewer import review_pr
...
async def test_review_pr_db_fail_continues_discord(db):
    ...
    with (
        patch("app.services.reviewer.get_pr_diff_text", return_value="diff text"),
        patch(
            "app.services.reviewer.client.messages.create",
...
async def test_review_pr_discord_fail(db):
    ...
    with (
        ...
        patch(
            "app.services.reviewer.send_discord_message",
            side_effect=Exception("Webhook URL 잘못됨"),
        ),
    ):
        from app.services.reviewer import review_pr
